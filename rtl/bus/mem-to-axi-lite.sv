...
module mem_to_axi_lite #(parameter int unsigned ADDR_W=32, parameter int unsigned DATA_W=32)
  ...
  assign axi.awaddr=addr_q; assign axi.awvalid=(state_q==WR_SEND);
  assign axi.wdata=data_q; assign axi.wstrb=be_q; assign axi.wvalid=(state_q==WR_SEND);
  ...
  always_ff @(posedge upstream.clk or negedge upstream.rst_n) begin
    if (!upstream.rst_n) state_q <= IDLE;
    else case (state_q)
      ...
      WR_SEND: if (axi.awvalid && axi.awready && axi.wvalid && axi.wready) state_q<=WR_RESP;
      WR_RESP: if (axi.bvalid && axi.bready) state_q<=IDLE;
